...
# dev.off()
#        pdf(file = 'prop0_.pdf', width = 11, height = 4)
...
for (comb.R.j in 1:length(comb.R.array)) {#           comb.R.j = 1
  ...
  #
  par(mar = c(3,2,2,1))
  plot(1,1, type = 'n', xlim = c(0, 1), ylim = c(0, 1), xaxs = 'i', yaxs = 'i', ann = F, axe = F)
  #
  ...
  #
  if(comb.R.j == 1){
    legend('bottomleft', lwd = 2, col = rev(col.for.k), legend = rev(paste0('dispersion = ', variable.k.array)), bty = 'n')
  }
  #
}
mtext(side = 1, line = 1, 'fraction of the fixed components', outer = T)
mtext(side = 2, line = 1, outer = T, 'proportion of infectious cases \nthat cause 0 secondary case', las = 0)
#      dev.off()
#      dev.off()
#      dev.off()
